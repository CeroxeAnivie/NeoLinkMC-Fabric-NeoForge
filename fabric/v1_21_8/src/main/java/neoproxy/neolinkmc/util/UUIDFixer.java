...
public class UUIDFixer {
     ...
     * @return UUID对象，解析失败返回null
     */
    private static UUID parseUUIDFromString(String uuidString) {
        NeoLinkMC.LOGGER.debug("[DEBUG] UUIDFixer.parseUUIDFromString() 被调用，字符串长度: {}", uuidString != null ? uuidString.length() : 0);

        // 防御性校验：API可能返回异常数据
        if (uuidString == null || uuidString.length() != 32) {
            NeoLinkMC.LOGGER.debug("[DEBUG] UUID字符串长度异常，期望32，实际: {}", uuidString != null ? uuidString.length() : "null");
            return null;
        }
        ...
        try {
            ...
            NeoLinkMC.LOGGER.debug("[DEBUG] UUID字符串解析失败: {} - {}", e.getClass().getName(), e.getMessage());
            return null;
        }
    }
}
